...
class Barbesrhop_Model(mesa.Model):
    ...
    def __init__(self, num_Barber):
        ...
        self.end_time_clock = hours * 60 + minutes
        ...
        self.max_num_in_waiting_room = 3
    ...
    def new_barber_signs_in(self):
        for agent in self.schedule.agents:
            if isinstance(agent, Barber) and agent.on_shift == False:
                ...
                print(f"[{str(self.convert_to_hh_mm(self.clock_time))}] Barber {self.num_Barber} ended shift")

    def create_customer(self):
        if self.time % 3 == 0:

            if self.num_in_waiting_room < self.max_num_in_waiting_room and self.clock_time < self.end_time_clock:
            ...
            elif self.num_in_waiting_room > self.max_num_in_waiting_room and self.clock_time < self.end_time_clock:
                self.customer_count += 1  # Increment the customer count for the next customer
                print(f"[{self.convert_to_hh_mm(self.clock_time)}] Customer {self.customer_count} leaves unfulfilled ")

            else:
                self.customer_count += 1  # Increment the customer count for the next customer
                print(f"[{self.convert_to_hh_mm(self.clock_time)}] Customer {self.customer_count} leaves disappointed ")
